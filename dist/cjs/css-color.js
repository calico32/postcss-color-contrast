...
const parseCssInt = (intOrPercentage) => {
    // int or percentage.
    if (intOrPercentage.at(-1) === '%')
        return clampByte((parseFloat(intOrPercentage) / 100) * 255);
    return clampByte(parseInt(intOrPercentage));
};
const parseCssFloat = (floatOrPercentage) => {
    // float or percentage.
    if (floatOrPercentage.at(-1) === '%')
        return clampFloat(parseFloat(floatOrPercentage) / 100);
    ...
    if (input.type === 'function') {
        ...
        switch (name) {
            ...
            case 'rgb':
                if (args.length !== 3)
                    return null;
                return [parseCssInt(args[0]), parseCssInt(args[1]), parseCssInt(args[2]), alpha];
            case 'hsla':
                if (args.length !== 4)
                    return null;
                alpha = parseCssFloat(args.pop());
